...
export default function App() {
  ...
  const [doubaoConfig, setDoubaoConfig] = useState<DoubaoConfig>({
    ...
    token: '',
    cluster: 'volcano_tts',
    voiceType: 'zh_female_qingxin_bigtts',
  });
  ...
  const onDone = useCallback(() => {
    if (articles.currentArticle) {
      articles.updateArticle(articles.currentArticle.id, {
  ...
  // Doubao TTS
  const onDoubaoSentenceChange = useCallback((index: number) => {
    const boundaries = doubaoTTS.sentenceBoundaries;
    if (boundaries[index] !== undefined) {
      progressRef.current = boundaries[index];
    }
  }, []);
  ...
  const doubaoTTS = useDoubaoTTS({
    ...
    volumeRatio: 1.0,
    pitchRatio: pitch,
    onSentenceChange: onDoubaoSentenceChange,
    onDone: onDoubaoDone,
  });
  ...
  const handlePlay = useCallback(() => {
    ...
    if (provider === 'doubao' && (!doubaoConfig.appId || !doubaoConfig.token)) {
      Alert.alert('Doubao Config Missing', 'Please enter your App ID and Access Token in Settings.');
      return;
    }
  ...
  const isSpeaking = activeTTS.isSpeaking;
  const isPaused = 'isPaused' in activeTTS ? activeTTS.isPaused : false;
    ...
    <View style={styles.root}>
      ...
      <View style={styles.providerBadge}>
        <Text style={styles.providerBadgeText}>
          {provider === 'doubao' ? 'Doubao TTS' : 'System TTS'}
        </Text>
      </View>
      ...
      {showList && (
        <View style={styles.modalOverlay}>
          <ArticleList
            ...
            currentId={articles.currentId}
            onSelect={articles.selectArticle}
            onDelete={articles.deleteArticle}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.settingsPanel}>
            ...
            <Settings
              ...
              onDoubaoConfigChange={setDoubaoConfig}
              selectedVoice={selectedVoice}
              voices={systemSpeech.voices}
