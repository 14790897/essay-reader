...
- **CI**: .github/workflows/ci.yml (check + build-web + deploy-web + release) and .github/workflows/android-build.yml (APK + smoke + instrumented)
- **Android directory**: Generated by `expo prebuild` in CI, NOT committed to git
...
- `android/` directory is gitignored — CI regenerates it via `expo prebuild`
- `test-run.js` expects `DOUBAO_API_KEY` env var, never hardcode keys
